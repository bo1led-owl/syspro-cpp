...
8 16-bit registers: rz, r1-r7.
Reads from `rz` are always zero, writes to `rz` are no-ops.

65536 memory cells (128KiB of memory).
...
Straightforward: `rz` is `000`, `r1`-`r7` is `001`-`111` respectively.
...
![Encoding tree](./encoding.png "Encoding tree")
